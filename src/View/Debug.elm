...
viewGrids model viewportWidth viewportHeight = div []
...
viewGrid model gridSize color viewportWidth viewportHeight =
    let
        ...
        startX = max 0 (floor (model.camera.x / gridSize)) * round gridSize
        startY = max 0 (floor (model.camera.y / gridSize)) * round gridSize
        endX = min model.mapConfig.width (model.camera.x + viewportWidth)
        endY =
...
viewPathfindingOccupancy model viewportWidth viewportHeight =
    if not model.showPathfindingOccupancy then
        ...
        let
            gridSize = model.gridConfig.pathfindingGridSize
            startPfX = max 0 (floor (model.camera.x / gridSize))
            startPfY = max 0 (floor (model.camera.y / gridSize))
            endPfX =
            ...
            cellsX = List.range startPfX endPfX
            cellsY = List.range startPfY endPfY
            allCells =
            ...
            renderCell ( x, y ) =
                let
                    worldX = toFloat x * gridSize
                    worldY = toFloat y * gridSize
...
viewBuildingOccupancy model viewportWidth viewportHeight =
    if not model.showBuildingOccupancy then
        ...
        let
            gridSize = model.gridConfig.buildGridSize
            startGridX = max 0 (floor (model.camera.x / gridSize))
            startGridY = max 0 (floor (model.camera.y / gridSize))
            endGridX =
                min (floor (model.mapConfig.width / gridSize)) (ceiling ((model.camera.x + viewportWidth) / gridSize))
            endGridY =
                min (floor (model.mapConfig.height / gridSize)) (ceiling ((model.camera.y + viewportHeight) / gridSize))
            cellsX = List.range startGridX endGridX
            cellsY = List.range startGridY endGridY
            allCells =
                List.concatMap (\x -> List.map (\y -> ( x, y )) cellsY) cellsX
            occupiedCells =
...
viewCityActiveArea model viewportWidth viewportHeight =
    if not model.showCityActiveArea then
        ...
        let
            gridSize = model.gridConfig.buildGridSize
            cityCells = getCityActiveArea model.buildings
            startGridX = max 0 (floor (model.camera.x / gridSize))
            startGridY = max 0 (floor (model.camera.y / gridSize))
            endGridX =
                min (floor (model.mapConfig.width / gridSize)) (ceiling ((model.camera.x + viewportWidth) / gridSize))
...
viewCitySearchArea model viewportWidth viewportHeight =
    if not model.showCitySearchArea then
        ...
        let
            gridSize = model.gridConfig.buildGridSize
            cityCells = getCitySearchArea model.buildings
            startGridX = max 0 (floor (model.camera.x / gridSize))
            startGridY = max 0 (floor (model.camera.y / gridSize))
            endGridX =
                min (floor (model.mapConfig.width / gridSize)) (ceiling ((model.camera.x + viewportWidth) / gridSize))
            ...
            case maybeUnit of
                Just unit ->
                    case unit.location of
                        OnMap x y ->
                            let
                                screenX = x - model.camera.x
                                screenY = y - model.camera.y
                                activeCircle = div
