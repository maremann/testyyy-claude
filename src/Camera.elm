...
constrainCamera config ( winWidth, winHeight ) camera =
    let
        viewWidth = toFloat winWidth
        viewHeight = toFloat winHeight
        minX = 0 - config.boundary
        maxX = config.width + config.boundary - viewWidth
        minY = 0 - config.boundary
        maxY = config.height + config.boundary - viewHeight
    in
    { x = clamp minX maxX camera.x
    , y = clamp minY maxY camera.y
    }
type alias MinimapConfig = { width : Int
    , height : Int
    , padding : Float
    }
...
isClickOnViewbox model minimapConfig clickX clickY =
    let
        ...
        viewboxLeft = minimapConfig.padding + (model.camera.x * scale)
        viewboxTop = minimapConfig.padding + (model.camera.y * scale)
        viewboxWidth = toFloat winWidth * scale
        viewboxHeight = toFloat winHeight * scale
    in
...
minimapClickOffset model minimapConfig clickX clickY =
    let
        ...
        viewboxLeft = minimapConfig.padding + (model.camera.x * scale)
        viewboxTop = minimapConfig.padding + (model.camera.y * scale)
        offsetX = clickX - viewboxLeft
        offsetY = clickY - viewboxTop
    in
    { x = offsetX, y = offsetY }
...
centerCameraOnMinimapClick model minimapConfig clickX clickY =
    let
        scale = getMinimapScale minimapConfig model.mapConfig
        ( winWidth, winHeight ) = model.windowSize
        terrainWidth = model.mapConfig.width * scale
        terrainHeight = model.mapConfig.height * scale
        clampedX =
...
minimapDragToCamera model offset clickX clickY =
    let
        ...
        scale = getMinimapScale minimapConfig model.mapConfig
        worldX = (clickX - minimapConfig.padding - offset.x) / scale
        worldY = (clickY - minimapConfig.padding - offset.y) / scale
    in
    { x = worldX, y = worldY }
